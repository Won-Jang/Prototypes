...
// Circular holes for fixing e.a.  X Y dia
T=[
[-15,10,4],   // trous coins  Corner holes
[-15,-10,4],
[15,10,4],
[15,-10,4]
];
//Trous rectangulaires X Y longX largY
// Rectangular holes X Y  Xlength  Ywidth
R=[[0,0,0,0]];
 
//Trous par STL   X Y ang nom du STL
//Holes by STL file    X Y  STLfile name
TS=[[0,0,0,0,"DSUB9.stl"]];

// Potentiometres    X Y angle calage, dia trou dia calage
// Potentiometers  X Y angle for stop  pot axis dia   dia for stop
TP=[
[10,0,90,0,0],
[50,0,45,0,0],
];
...
// Servomotors position (center X,Y, angle)
S=[[-100,-100,90]];  // here out of plate for NO servos
...
difference() {
  union() {  
    ...
    // trous pour potars
    for(N=[0:len(TP)-1]) {  
        Pot(TP[N][0],TP[N][1],TP[N][2],TP[N][3],TP[N][4]);  
    }  // for N
